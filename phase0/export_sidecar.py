...
def _export_residency(th: dict, grid: dict, args) -> dict:
    """teal_residency sidecar: per-layer gate_select tau + the residency knobs the M3 engine reads
    (density, hot_cache_gb, ring_gb, io_budget_clusters). The runtime falls back to an on-device
    histogram top-K for any layer whose tau is missing/<=0, so gate_select is best-effort."""
    gate_select: dict[str, float] = {}
    for layer_key, classes in grid.items():
        if "gate_select" in classes:
            gate_select[str(layer_key)] = float(classes["gate_select"])
    out = {
        "mode": "teal_residency",
        "density": args.density,
        "hot_cache_gb": args.hot_cache_gb,
        "ring_gb": args.ring_gb,
        "io_budget_clusters": args.io_budget_clusters,
        "gate_select": gate_select,
    }
    print(f"wrote {args.out}  (mode=teal_residency, density={args.density}, "
          f"hot_cache_gb={args.hot_cache_gb}, io_budget_clusters={args.io_budget_clusters}, "
          f"gate_select_taus={len(gate_select)})", file=sys.stderr)
    return out
...
def main() -> None:
    ...
    ap.add_argument("--mode", default="teal", help="sparse mode: teal | teal_residency")
    ap.add_argument("--out", default="clarity-sparse.json", help="output sidecar path")
    # teal_residency knobs (ignored for teal)
    ap.add_argument("--density", type=float, default=0.40, help="residency select density (default 0.40)")
    ap.add_argument("--hot-cache-gb", dest="hot_cache_gb", type=float, default=1.05)
    ap.add_argument("--ring-gb", dest="ring_gb", type=float, default=0.25)
    ap.add_argument("--io-budget-clusters", dest="io_budget_clusters", type=int, default=24)
    args = ap.parse_args()
    ...
    if args.mode == "teal_residency":
        out = _export_residency(th, grid, args)
